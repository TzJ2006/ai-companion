...
import type { FunctionModularity } from "./types.js";

export function buildContractFileContent(functions: FunctionModularity[]): string {
  const lines: string[] = [
...
export async function emitContractFiles(
  functions: FunctionModularity[],
  outputDir: string
): Promise<string[]> {
