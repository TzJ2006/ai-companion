...
function findTsWasmPath(): string {
  ...
  throw new Error(
    `tree-sitter-typescript.wasm not found. Searched:\n${candidates.join("\n")}`
  );
}
...
function extractTsParams(node: SyntaxNode): FunctionParam[] {
...
function extractTsClass(node: SyntaxNode): ClassInfo {
  const nameNode = node.childForFieldName("name");
...
function extractTsDecorators(node: SyntaxNode): string[] {
...
function extractJsDoc(node: SyntaxNode): string | null {
...
function extractTsImport(node: SyntaxNode): ImportInfo {
