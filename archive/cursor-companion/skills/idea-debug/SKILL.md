...
name: idea-debug
description: >-
  Debugs a failing idea by first listing every line-referenced mismatch between
  ideas/graph.yaml and the code, then — only if the code matches the idea —
  showing real inputs/outputs so the human can revise the idea. Use when a
  node is blocked, a verify fails, implementation disagrees with the idea,
  文档和实现不一致, or the user says debug, fix, or /idea-debug. Do not use for
  a new idea (/idea-discuss) or a first implementation (/idea-build).
icon: bug
color: orange
---
...
After the human says the code is wrong: unlock it, then fix.

```bash
npx tsx .cursor/companion/ideas.ts set <id> doing --by idea-debug --note "code vs idea, fixing code"
```

The write-gate will only allow files listed on this idea. Then run
`verify.command`, then the wider suite. If it now passes, mark it `done`
(that run is required — `--force` only if the human asked).
...
log the revision, and hand back to `/idea-build` — which will rewrite the test
...
```bash
npx tsx .cursor/companion/ideas.ts log --by idea-debug --ideas I-014 --files src/a.ts --note "step 1 code-wrong|idea-stale / step 2 idea-wrong; root cause in one line"
```

Also append a `log:` entry on the idea itself (`ideas.ts set` does this) — the
reason belongs next to the idea, where the next person will actually read it.
