export function renderSessionToHtml(session, _rawDiffs, options = { show_test_status: true, show_error_ids: true, style: "side-by-side" }) {
...
function groupByReason(changes) {
    const map = new Map();
...
function renderReasonGroup(group, index) {
...
function renderFileSection(filePath, changes) {
...
function renderInlineDiff(oldContent, newContent) {
...
function buildPage(title, session, groupsHtml) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
