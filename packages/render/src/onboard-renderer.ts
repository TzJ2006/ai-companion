...
import type { ParsedModule, FunctionSignature, ClassInfo } from "@aidev/ast";
...
function renderOverview(totalFunctions: number, totalFiles: number, modules: ParsedModule[], reportData?: ReportData): string {
...
function renderModules(modules: ParsedModule[], reportData?: ReportData): string {
  const sorted = [...modules].sort((a, b) => a.file_path.localeCompare(b.file_path));

  const reasonMap = new Map<string, FunctionReasonData>();
  if (reportData) {
  ...
  const cards = sorted.map(mod => {
    ...
    const counts: string[] = [];
    if (mod.functions.length > 0) counts.push(`${mod.functions.length} functions`);
    if (mod.classes.length > 0) counts.push(`${mod.classes.length} classes`);
    if (mod.imports.length > 0) counts.push(`${mod.imports.length} imports`);
...
function renderFunctionCard(fn: FunctionSignature, reasonData?: FunctionReasonData): string {
...
function renderClassCard(cls: ClassInfo, filePath: string, reasonMap: Map<string, FunctionReasonData>): string {
...
function renderFunctionIndex(functionIndex: Record<string, FunctionIndexEntry>): string {
