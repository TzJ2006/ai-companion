...
function extractParams(node: SyntaxNode): FunctionParam[] {
...
function extractClass(node: SyntaxNode): ClassInfo {
  const nameNode = node.childForFieldName("name");
  const bodyNode = node.childForFieldName("body");
...
function extractDocstring(bodyNode: SyntaxNode | null): string | null {
...
function extractImport(node: SyntaxNode): ImportInfo {
  const names: string[] = [];
  for (let i = 0; i < node.namedChildCount; i++) {
    const child = node.namedChild(i)!;
...
function extractFromImport(node: SyntaxNode): ImportInfo {
