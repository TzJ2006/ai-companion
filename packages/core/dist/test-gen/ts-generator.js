export function generateTestSkeleton(mod, config) {
    const results = [];
...
function buildFunctionTest(fn, sourceFile, config) {
...
function buildClassTest(className, methods, sourceFile, config) {
    const importPath = sourceFile.replace(/\.ts$/, ".js");
    const lines = [];
    const fw = config.test_framework;
...
function inferMockValue(name, type) {
    if (!type)
    ...
    if (t === "boolean" || t === "bool")
        return "true";
    if (t.startsWith("string[]") || t === "array<string>")
        return `["a", "b"]`;
    if (t.startsWith("number[]") || t === "array<number>")
        return "[1, 2, 3]";
    if (t.includes("[]") || t.startsWith("array"))
        return "[]";
    if (t.includes("map") || t.includes("record"))
        return "{}";
    if (t.includes("promise"))
        return inferMockValue(name, t.replace(/promise<(.+)>/, "$1"));
...
function inferFromName(name) {
    ...
    if (lower.includes("name") || lower.includes("label") || lower.includes("title"))
        return `"test-name"`;
    if (lower.includes("id") || lower.includes("hash"))
        return `"abc123"`;
    if (lower.includes("count") || lower.includes("num") || lower.includes("index") || lower.includes("limit"))
        return "10";
    if (lower.includes("flag") || lower.includes("enabled") || lower.includes("active"))
        return "true";
    if (lower.includes("options") || lower.includes("config") || lower.includes("opts"))
        return "{}";
    if (lower.includes("items") || lower.includes("list") || lower.includes("entries"))
        return "[]";
    if (lower.includes("callback") || lower.includes("fn") || lower.includes("handler"))
        return "() => {}";
    if (lower.includes("source") || lower.includes("content") || lower.includes("text"))
        return `"test content"`;
    return `undefined /* TODO: provide ${name} */`;
}
function inferReturnTypeCheck(returnType) {
...
function inferCanThrow(fn) {
    if (fn.docstring?.toLowerCase().includes("throw"))
        return true;
    if (fn.docstring?.toLowerCase().includes("error"))
        return true;
    if (fn.name.includes("parse") || fn.name.includes("Parse"))
        return true;
    if (fn.name.includes("validate") || fn.name.includes("Validate"))
        return true;
    if (fn.name.includes("read") || fn.name.includes("Read"))
        return true;
    if (fn.name.includes("load") || fn.name.includes("Load"))
        return true;
    const hasNullableParams = fn.params.some(p => p.type?.includes("null") || (!p.type && !p.default_value));
    return hasNullableParams;
}
function inferBadValue(_name, type) {
    if (!type)
        return "null as any";
...
function buildTestPath(sourceFile, name, _className, config) {
    const baseName = sourceFile.split("/").pop()?.replace(/\.ts$/, "") ?? "module";
    return `${config.output_dir}/test_${baseName}_${name}.test.ts`;
}
export function buildLlmEnhancePrompt(fn, sourceBody, skeleton) {
