...
Read `claude-companion/FORMAT.md` first. It defines the node format; do not invent your own.
...
```bash
npx tsx claude-companion/ideas.ts scan          # 建清单 + 看还剩多少
npx tsx claude-companion/ideas.ts scan --n 0    # 打印全部剩余
```
...
open questions. See the Recording section of `claude-companion/README.md`.
