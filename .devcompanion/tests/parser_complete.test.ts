...
const TMP_DIR = resolve(tmpdir(), "parser-complete-tests");
let tmpCounter = 0;
...
describe("Parser - initParser", () => {
  ...
  it("should be idempotent on subsequent calls", async () => {
    await initParser();
    await expect(initParser()).resolves.toBeUndefined();
  });
});
...
describe("extractFunction - Individual Function Tests", () => {
  it("should extract simple function name", async () => {
  ...
  it("should extract return type annotation", async () => {
    const source = "def get_number() -> int:\n    return 42\n";
    const result = await parseFromSource(source);
  ...
  it("should handle function with no return type", async () => {
    const source = "def no_return():\n    pass\n";
    const result = await parseFromSource(source);

    expect(result.functions[0].return_type).toBeNull();
  });

  it("should set class_name for methods", async () => {
    const source = "class MyClass:\n    def my_method(self):\n        pass\n";
    const result = await parseFromSource(source);

    expect(result.classes[0].methods[0].class_name).toBe("MyClass");
  });
});

describe("extractParams - Individual Parameter Tests", () => {
  it("should extract simple parameter", async () => {
    const source = "def func(x):\n    pass\n";
    const result = await parseFromSource(source);

    expect(result.functions[0].params).toHaveLength(1);
    expect(result.functions[0].params[0].name).toBe("x");
  });

  it("should mark parameters without type as null type", async () => {
    const source = "def func(x):\n    pass\n";
    const result = await parseFromSource(source);

    expect(result.functions[0].params[0].type).toBeNull();
  });

  it("should mark parameters without default as null default", async () => {
    const source = "def func(x: int):\n    pass\n";
    const result = await parseFromSource(source);

    expect(result.functions[0].params[0].default_value).toBeNull();
  });
});

describe("extractClass - Individual Class Tests", () => {
  ...
  it("should extract class with no bases", async () => {
  ...
  it("should extract empty methods list for empty class", async () => {
    const source = "class Empty:\n    pass\n";
    const result = await parseFromSource(source);
  ...
  it("should mark methods as is_method true", async () => {
    const source = "class C:\n    def m(self):\n        pass\n";
    const result = await parseFromSource(source);

    expect(result.classes[0].methods[0].is_method).toBe(true);
  });
});

describe("handleDecorated - Decorator Extraction", () => {
  it("should extract decorated function", async () => {
  ...
  it("should handle multiple decorators", async () => {
    ...
    expect(result.functions[0].decorators).toContain("decorator1");
    expect(result.functions[0].decorators).toContain("decorator2");
  });

  it("should handle decorated method in class", async () => {
    const source = "class C:\n    @staticmethod\n    def m():\n        pass\n";
    const result = await parseFromSource(source);

    expect(result.classes[0].methods[0].decorators).toContain("staticmethod");
  });
});

describe("extractDecorators - Specific Decorator Tests", () => {
  it("should extract decorator name without @ symbol", async () => {
    const source = "@my_decorator\ndef func():\n    pass\n";
    const result = await parseFromSource(source);

    const decorators = result.functions[0].decorators;
    expect(decorators[0]).toBe("my_decorator");
  ...
  it("should extract multiple decorators", async () => {
    ...
    expect(result.functions[0].decorators).toHaveLength(3);
  });
...
describe("extractDocstring - Docstring Extraction", () => {
  ...
  it("should extract triple-quoted docstring", async () => {
    const source = "def func():\n    '''Another docstring'''\n    pass\n";
    const result = await parseFromSource(source);

    expect(result.functions[0].docstring).toBe("Another docstring");
  });

  it("should return null when no docstring", async () => {
    const source = "def func():\n    pass\n";
    const result = await parseFromSource(source);
...
describe("extractImport - Simple Import Tests", () => {
  it("should extract simple import module name", async () => {
    const source = "import os\n";
    const result = await parseFromSource(source);
  ...
  it("should extract line number", async () => {
    const source = "import os\n";
    const result = await parseFromSource(source);
  ...
  it("should extract names array", async () => {
    const source = "import os\n";
    const result = await parseFromSource(source);

    expect(Array.isArray(result.imports[0].names)).toBe(true);
  });
});

describe("extractFromImport - From Import Tests", () => {
  it("should extract from import module", async () => {
    const source = "from os import path\n";
    const result = await parseFromSource(source);
  ...
  it("should extract imported names", async () => {
    const source = "from os import path, getcwd\n";
    const result = await parseFromSource(source);

    const imp = result.imports[0];
    expect(imp.names.length).toBeGreaterThanOrEqual(2);
  });

  it("should extract line number for from import", async () => {
    const source = "from os import path\n";
    const result = await parseFromSource(source);
  ...
  it("should handle from ... import star", async () => {
