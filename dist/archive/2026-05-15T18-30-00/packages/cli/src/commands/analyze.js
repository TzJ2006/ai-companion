...
function extractSourceBody(fileContent, startLine, endLine) {
    const lines = fileContent.split("\n");
    return lines.slice(startLine - 1, endLine).join("\n");
}
function buildSignatureString(fn) {
...
export const analyzeCommand = new Command("analyze")
    ...
    for (const file of files) {
        try {
            ...
            console.warn(`  Warning: ${relative(projectRoot, file)}: ${e.message}`);
        }
    }
    ...
    if (opts.modularity) {
        ...
        const modInputs = inputs.map(inp => ({
