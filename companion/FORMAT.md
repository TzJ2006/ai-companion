...
- id: I-014
  ...
  blocked_because: 等 I-008 的接口定下来   # required while blocked (I-133): why it stopped, what restarts it
...
- **修订（2026-09-16，I-147）：这条只管项目根之下的文件。** 落在操作系统临时目录里的
  路径放行 —— 那是草稿区，不是任何项目的产品文件；落在项目外别处的路径仍拒，但理由
  说的是「这张图不管项目外的文件」，不再冒充「没有想法认领」。之前项目外路径一律掉进
  D16 的拒绝，把会话的临时目录也拦了，人只好绕。
...
- **落地情况（2026-09-16，I-115）：想法之间不重叠，会话之间也不重叠。** 多个助手会话在
  同一目录里并行时，光靠想法的 `code.file` 分不开「谁在写」—— 两个会话可以同时把同一个
  想法 `set doing`。所以在 `coord enable` 打开的协作模式下，守卫写前多问一句：这个文件
  现在归哪个会话（I-114 的认领）。持有者放行；别人持有、没人认领、或宿主没带会话身份，
  一律拒并说出下一步（联系持有者 / 先 claim / 这条路只能合作式用 coord 命令）。身份只认
  宿主 hook 事件里报的：Claude 的 session_id（加子助手的 agent_id）、Cursor 的
  conversation_id（generation_id 每轮变，不算身份）、Codex 的 session_id —— Codex 官方说子助手
  继承父会话号，所以 Codex 的父子分不开，`coord status` 明写这一点。
- **修订（2026-09-16，I-149）：引用不等于占用，想法之间可以重叠。** `code.file` 答的是
  「实现在哪」，多个想法引用同一个文件是常态（父想法收尾、只做验收、修同一处的两个小改动）；
  拿它当「谁在写」的互斥，挡住的是不写文件的验收，护不住真正并行的两个会话。所以
  `set doing` 不再因文件重叠拒绝，只把重叠的对方记进 log。写互斥只剩一处、也只需一处：
  协作模式下守卫在写那一刻按会话认领核对（上一条落地情况）。单会话没有可争的；同目录多会话
  不开 `coord enable` 就没有保护——这是明说的边界，不是遗漏。写前核对不比对文件内容是否
  自读取后被改过：编辑工具的旧文本匹配已覆盖工具写，shell 写不覆盖，出过覆盖事故再加。
...
- **协作模式下想法图禁止裸写（2026-09-16，I-115）** — `coord enable` 之后，经 hook 的
  编辑工具写 `ideas/graph.yaml` 一律拒：几个会话同时改同一份 YAML 会互相覆盖，而 I-116 的
  `edit` / `apply` / `set` 走的是带锁的事务。协调记录（`.runtime/coord/`）本来就是 CLI 专属。
  守卫管得到的只有经过 hook 的写，这句话写在拒绝理由和 `coord status` 里，不冒充文件锁。
...
- **裁决** — 基座提供 `paths init migrate new edit check status next show set allow scan
...
- **理由** — `paths` 消除猜文件，`allow` 可在写前自检，approval/run-check 承载
  ...
  继续增加同义词没有价值。
- **落地情况（更新于 2026-09-16）** — 上面裁决里的每一条都已落地，**连同 `log` 一共
  十九条**：`paths init migrate scan new edit check status next show log set allow
  render apply serve request-approval run-check coord`。`coord` 提供会话注册、消息、阅读确认、
  文件认领与短锁恢复（I-113/I-114）；`edit` 是 I-116 加的局部改字段入口（带旧值比对，见 D30
  落地情况）；宿主写前的归属约束尚待 I-115。`log` 把想法自带的那份只追加
...
- **落地情况（2026-09-16，I-116）：写图只剩一个事务边界。** 引擎里的 `mutateGraph` /
  `mutateGraphText` 把「拿项目短锁（I-113 的 `withProjectLock`）—— 在锁里读当前文件 ——
  改 —— 原子写回 —— 渲染网页 —— 放锁」包成一步；`set`、`new`、`edit`、`apply`、`serve`
  的 `POST /changes`、`applyApproval` 的人工签字回写、独立 `render` 全走它。之前每条路都
  是启动时读一份、末尾写一份，两个进程从同一份旧图出发就互相盖；现在锁只占提交那几毫秒，
  不占任务。新增 `edit <id> --field --value-json [--old-json]`：局部提交一个字段，给了旧值
  就要和当前值一致，不一致明确拒绝 —— 同一字段两人各从旧视图改，恰好一个赢。字段限
  八问散文、name、parent、needs、code、verify、blocked_because；`status` 仍走 `set`，
  `signed_off` 仍只能由人回口令（D24/D27）；会新增 `check` 错误的改动（成环、未知前置）
  不落盘。渲染失败时 YAML 已提交，返回的是「图已保存，网页需重建」，不冒充整体失败。
  诚实的边界：YAML、HTML、回执仍是多个文件，不承诺掉电时一起提交；YAML 是可重渲染的真相。
- **落地情况（2026-09-16，I-115）：认领接进了写门。** 事务边界防的是「两个进程写同一份图」；
  认领防的是「两个会话改同一个产品文件」。后者在协作模式下由守卫写前核对（见 D18 落地情况），
  放行的写顺带把该会话的未读协作消息以 additionalContext / agent_message 送进上下文，标明是
  别的会话说的话、不是指令，读完要自己 `coord ack`；Stop 时还持有认领只提醒不拦。
...
- **裁决** — 共同基座保留五个标准 Agent Skills；共用正文和 references，各平台
  ...
  **正文是合并不是重写（2026-09-02 修订）**：共同技能的正文由三家前身合并而来，
  前身里的护栏逐条保留，删任何一条都要在本表留痕。历史要求包括：ccfix 的「不一致
  清单必须作为独立可见的消息交给人并结束回合」「一次只修一处、分别验证」「三次
  修不好转 blocked」「收工前拆掉临时插桩」（停顿与三次规则已由下方 I-148 修订）；ccscan 的「找不到真实理由就写
...
### 目标授权、提问与交接（2026-09-16，I-148）

本节取代技能中的固定阶段确认：ccthink 的复述、拆分、全案各等一次确认，ccfix 的
发现差异就结束回合、三次失败就交给人，以及 ccscan 的报告后必须等审，不再作为通用要求。
用户已授权完成一个目标时，助手持续推进范围内的阅读、规划、实现、验证和普通修复。
仅请求规划、扫描、查看状态不等于授权写产品代码；沉默不产生授权。不得因为流程换阶段、
切换技能或新建节点重复索取已有授权。平台权限、文件认领、GREEN 和人工签字照旧。

**何时提问。** 目标或预期结果存在影响交付的歧义；需要扩大授权范围；存在尚未授权的
删除数据、对外发布、费用等重要后果；或者确实只有人能验收。已经明确授权的同一动作不重复问。
常规技术选择、图与实现的明确偏差、可调查的失败由助手处理；连续三次失败先复盘证据，
没有新证据就停止重复尝试，记录受阻原因。等待答案时继续不依赖该决定的工作。

**问什么。** 每次给出具体决定或可验收的结果、为什么必须由人判断、选项及后果、建议，
说明什么已完成、什么在等答案。人工验收给可打开的路径、操作步骤和预期现象，再给
manual-check 口令。不要只递交节点编号、八问全文或“是否批准计划”。

**交接记进 log。** 阶段切换和交接前，在相关想法现有的 `log` 列表中追加一条摘要，内容就是下一节
「交付三段」的三件事外加已有决定：本次目标与完成标志、已完成的事及到了哪一级、下一步、人现在要
做什么、已有决定及其范围与前提、待决事项（没有就写“暂无”）。保留必要文件位置、验证结果与用户决定；
不另建一份待办或状态系统，不用每次复制全部八问。新会话先读当前用户指令、相关想法及最近的摘要，
核对实际文件和验证状态后继续；`log <id> --n 3` 可读取最近记录，不能用来追加。记录过时或缺失时
明确说明，不凭状态猜进度。只读查看不追加记录。

### 主线、交付三段、拒绝分类与记录分档（2026-09-16，I-148 第二轮：I-150～I-156）

**执行中进度块（I-156，2026-09-16 修订）。** 转向、受阻、阶段切换、拿到验证结果时，在对话里贴一次固定
进度块，其他时候不刷：
```
主任务： 这次委托要完成什么（不是眼前的障碍）
当前情况： 刚确认了什么、现在卡在哪
正在做： 这一步是什么，为什么它有助于主任务
已完成： 做到哪了
还差什么： 剩下哪些
下一步： 做完这步回到哪
需要你决定： 暂无 / 一个讲清原因、选项、后果、建议的具体问题
```
第一行永远是用户委托的总目标；局部排障不得替换它，转向前先判断它是必要前置、可选改进还是无关问题，
无关的记下不做。贴完继续做——只有「需要你决定」不是「暂无」时才停；告知不是批准关卡。

**交付三段（I-156）。** 阶段交付、受阻、结束回复固定用三个部分，顺序不变，取代此前的「工作主线」
五项和「进度四项」：
1. **这次完成了什么** —— 用结果说，不罗列工具调用；每项标到哪一级：代码已修改 / 验证已通过 /
   已安装生效 / 真实使用已验证；说明与总目标的关系和尚未验证的限制。计划、部分完成、只有测试通过，
   都不能写成问题已解决。
2. **之后 agent 还要做什么** —— 按顺序，紧接着的一步写在最前；已授权且能继续的直接继续，不问
   「是否继续」；内部调查、常规实现选择、测试、协调归 agent，包括需要另一个会话或另一台机器的测试，
   由 agent 说明怎么安排，不交给人组织；无法继续时写具体阻碍和仍可推进的独立工作；全部完成时写
   「无剩余工作」，不凭空加任务。
3. **人现在需要做什么** —— 不需要就明写「你现在不需要操作，我会继续……」，全部完成则写
   「你现在不需要操作，可以开始使用……」；需要时给入口、按顺序的步骤、可复制的命令或文字、预期结果、
   完成后把什么反馈给 agent，并分「现在必须做」和「可选」；不说「开个新会话测试一下」「你提交一下」，
   不让人翻上一条消息。需要人决定的，只问已有授权定不了的新选择，附原因、后果和建议，
   不把内部技术方案交给人选。
不为展示格式提前结束还能继续的工作；不承诺回复结束后会自动继续，除非确实安排了可执行的后续机制。
会话开头的简报（I-152）带一行「需要人操作」，列出进行中且等人签字的想法；没有就写「无」。

三种情形各一份对照回复，写法以此为准：
- 未完成、可自主继续：「完成了什么：守卫的 Stop 分流已改，测试通过，尚未装进本仓库。agent 接着做：
  构建、装到本仓库、用一次只读会话实测。你现在不需要操作，我会继续装和测。」
- 需要人做一件事：「完成了什么：I-130 页面四项行为代码已改、测试通过、已安装生效，只剩人工验收。
  agent 接着做：等你反馈后关掉 I-130。你现在必须做：打开 ideas/graph.html 点进 I-130，看到标题下
  三条要点、展开一行能看到子想法；符合就整条回复 `批准 CC-XXXXXXXX`，不符合就说哪一条不对。」
- 全部完成：「完成了什么：一行 README 已改、测试通过、已安装无关。无剩余工作。你现在不需要操作，
  可以开始使用；要留在历史里就提交，命令：`git commit -m "…"`。」

**拒绝分四类，谁处理写在最前面。** 守卫每条拒绝理由带类别前缀（I-151）：
【自行处理】缺认领、命令用法、状态转换、普通测试失败 —— 助手自己查、自己修，不交给人；
【缺授权】提交历史、下载落地、扩大范围 —— 向人提出具体决定、后果和建议；
【能力受限】守卫禁止助手驱动本地服务、项目外文件、hook 入口 —— 说清准确缺口和替代路径，
不说成「只有人能判断」；【需人判断】人工签字、主观验收 —— 给可打开的结果和步骤，请人判断。
类别是守卫按理由文字判的，是否真要人参与，助手还要结合目标和已有授权再判一次。

**已有决定连内容带范围一起保留。** log 里记：同意了什么结果、是否授权实现、范围与约束、
前提是什么。再次提问必须说明比上次多了什么新选择或后果；没有新增就继续执行。

**记录分三档（I-153）。** 小改动：what、why、code.file、verify 即可开工，一条
`new <名称> --what … --why … --code 文件 --verify 命令` 建好；有实质行为变化：补 expected、how
与边界条件；大功能：完整八问与拆解。意图来自用户对话，实现位置来自代码，验证来自实际结果，
缺失不编造；尽量引用已有想法，不建长期不关闭的「小修小补」节点。缺的四问由 `check` 提示。

**只读问答不被历史错误拦住（I-150）。** Stop 只拦本次会话改过或影响到的想法（写过的文件所属
想法、引擎命令点名的想法，及依赖它们的想法和它们的父想法）上的错误；其余作为历史提示。
会话开头只给进行中想法的最近记录、受阻数和可开工数，整张表一条命令可查（I-152）。
