...
npm run test           # vitest WATCH mode — never exits; use `npx vitest run` for one-shot
...
# Install/manage AI Dev Companion in other projects (idempotent; registry-tracked)
...
- **/ccoverview** — thin wrapper around `scripts/generate-overview.ts` for the *current* project (never `--target`). Default emits bilingual `overview-en.html` + `overview-zh.html` (Claude CLI translation); `--skip-translation` emits a single `overview.html` with no LLM call.
- **/idea** — idea backlog + research (`/idea add|list|research|show`), stored at `.devcompanion/ideas/`.
