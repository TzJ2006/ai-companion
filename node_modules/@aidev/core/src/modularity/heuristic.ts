...
function detectResponsibilities(source: string): string[] {
...
export function computeCohesion(source: string, _name: string): CohesionMetrics {
...
export function computeCoupling(source: string, imports: string[]): CouplingMetrics {
...
export function detectHiddenDependencies(source: string): HiddenDependency[] {
  const deps: HiddenDependency[] = [];
  const lines = source.split("\n");

  const patterns: ReadonlyArray<[RegExp, HiddenDependency["kind"]]> = [
...
export function computeInterfaceClarity(input: ModularityInput): InterfaceClarity {
  const issues: string[] = [];
...
export function generateContract(input: ModularityInput, source: string): InterfaceContract {
...
export function analyzeModularityHeuristic(input: ModularityInput): FunctionModularity {
