...
export const onboardCommand = new Command("onboard")
    ...
    for (const file of files) {
        try {
            ...
            console.warn(`  Warning: Failed to parse ${relative(projectRoot, file)}: ${e.message}`);
        }
    }
    const index = {
    ...
    if (opts.tests !== false) {
        ...
        for (const { module: mod } of fileResults) {
            if (mod.functions.length === 0 && mod.classes.length === 0)
                continue;
...
async function enhanceTestsWithLlm(testDir, fileResults) {
